...
3. Создать файл `.env` на подобии `example.env` и изменить под свои нужды. Для теста приложения можно пропустить этот шаг, будут использоватся переменные из `example.env`
